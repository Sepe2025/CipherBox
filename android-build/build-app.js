...
if (!fs.existsSync(KS)) {
  sh('keytool', ['-genkeypair', '-keystore', KS, '-alias', 'vault', '-keyalg', 'RSA',
    ...
    '-storepass', '<REDACTED-RETIRED-SIGNING-PASSWORD>', '-keypass', '<REDACTED-RETIRED-SIGNING-PASSWORD>',
    '-dname', 'CN=Vault, OU=Local, O=G, C=CN'], '生成签名密钥（首次）');
}

/* 8. 签名 + 对齐确认 */
shBat(APKSIGNER, ['sign', '--ks', KS, '--ks-pass', 'pass:<REDACTED-RETIRED-SIGNING-PASSWORD>', '--key-pass', 'pass:<REDACTED-RETIRED-SIGNING-PASSWORD>',
