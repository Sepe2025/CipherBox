...
const OUT = path.join(__dirname, 'shots', 'hscroll');
...
(async () => {
  ...
  const profile = path.join(OUT, '..', 'hs-' + port);
  ...
  /* 滚到顶 */
  await ev(`(function(){ var sh=document.querySelector('#settingsOverlay .sheet'); sh.scrollTop=0; })()`);
  await sleep(300);
  await shot('hs-top.png');
  await shot('hs-top-zoom.png', { x: 0, y: 300, width: 390, height: 380, scale: 2 });

  /* 滚到底（使用说明可见）*/
  await ev(`(function(){ var sh=document.querySelector('#settingsOverlay .sheet'); sh.scrollTop=sh.scrollHeight; })()`);
  await sleep(300);
  await shot('hs-bottom.png');
  await shot('hs-bottom-zoom.png', { x: 0, y: 300, width: 390, height: 380, scale: 2 });

  /* 量化：sheet 内不同高度的水平亮度扫描（找横向明暗分界）*/
  const scan = await ev(`(function(){
    var sh=document.querySelector('#settingsOverlay .sheet');
    sh.scrollTop=sh.scrollHeight;
    var r=sh.getBoundingClientRect();
    var c=document.createElement('canvas'); c.width=390; c.height=Math.round(r.height);
    var ctx=c.getContext('2d');
    var out=[];
    /* 用 elementFromPoint 不可行——改为逐行取 sheet 的合成亮度不可行（无像素访问）。
       退而求其次：输出 sheet 的滚动与几何信息供分析 */
    return JSON.stringify({ sheetTop: Math.round(r.top), sheetH: Math.round(r.height),
      scrollH: sh.scrollHeight, scrollTop: sh.scrollTop });
  })()`);
  console.log('  [几何] ' + scan);
