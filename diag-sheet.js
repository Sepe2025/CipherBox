/* 弹层玻璃「分层」诊断：同一弹层，渐变方案 vs 临时恢复 blur(132px) 对比 */
'use strict';
...
const OUT = path.join(__dirname, 'shots', 'sheet-diag');
...
(async () => {
  ...
  const profile = path.join(OUT, '..', 'sd-' + port);
  ...
  const setBlur = async (on) => {
    await ev(`(function(){
      var st=document.getElementById('__diag'); if(!st){ st=document.createElement('style'); st.id='__diag'; document.head.appendChild(st); }
      st.textContent = ${JSON.stringify(on)} ? '.aurora i{ filter:blur(132px) !important; } .aurora i:nth-child(4){ filter:blur(132px) !important; }' : '';
    })()`);
    await sleep(300);
  };
  ...
  await sleep(500);
  await setBlur(false);
  await shot('settings-gradient.png');
  await setBlur(true);
  await shot('settings-blur132.png');
  await ev(`document.querySelector('#settingsClose').click()`);
  await sleep(300);

  console.log('=== 添加（编辑）弹层 ===');
  await ev(`document.querySelector('#fab').click()`);
  await sleep(500);
  await setBlur(false);
  await shot('edit-gradient.png');
  await setBlur(true);
  await shot('edit-blur132.png');
  await ev(`document.querySelector('#editClose').click()`);
  await sleep(300);

  console.log('=== 更多菜单弹层 ===');
  await ev(`(function(){ var c=document.querySelector('#mainList .card'); if(c) c.querySelector('[data-act="more"]').click(); })()`);
  await sleep(500);
  await setBlur(false);
  await shot('more-gradient.png');
  await setBlur(true);
  await shot('more-blur132.png');
